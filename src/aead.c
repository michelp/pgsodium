...
pgsodium_crypto_aead_ietf_noncegen(PG_FUNCTION_ARGS)
{
	...
	bytea* result = _pgsodium_zalloc_bytea(result_size);
	randombytes_buf(VARDATA(result), crypto_aead_chacha20poly1305_IETF_NPUBBYTES);
	PG_RETURN_BYTEA_P(result);
}
...
pgsodium_crypto_aead_ietf_encrypt(PG_FUNCTION_ARGS)
{
	...
	bytea* result;
	ERRORIF(VARSIZE_ANY_EXHDR(nonce) != crypto_aead_chacha20poly1305_IETF_NPUBBYTES,
			"invalid nonce");
	ERRORIF(VARSIZE_ANY_EXHDR(key) != crypto_aead_chacha20poly1305_IETF_KEYBYTES,
			"invalid key");
	
	result_size =  VARSIZE_ANY(message) + crypto_aead_chacha20poly1305_IETF_ABYTES;
...
pgsodium_crypto_aead_ietf_decrypt(PG_FUNCTION_ARGS)
{
	...
	ciphertext_len = VARSIZE_ANY_EXHDR(ciphertext) - crypto_aead_chacha20poly1305_IETF_ABYTES;
...
pgsodium_crypto_aead_ietf_encrypt_by_id(PG_FUNCTION_ARGS)
{
	...
	ERRORIF(VARSIZE_ANY_EXHDR(nonce) != crypto_aead_chacha20poly1305_IETF_NPUBBYTES,
			"invalid nonce");
	key = pgsodium_derive_helper(key_id, crypto_aead_chacha20poly1305_IETF_KEYBYTES, context);
	result_size =  VARSIZE_ANY(message) + crypto_aead_chacha20poly1305_IETF_ABYTES;
	result = _pgsodium_zalloc_bytea(result_size);
	crypto_aead_chacha20poly1305_ietf_encrypt(
		PGSODIUM_UCHARDATA(result),
